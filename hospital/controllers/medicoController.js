...
const medicoController = {
  ...
  index: async (req, res) => {
    try {
      ...
      if (especialidad && especialidad !== '') where.id_especialidad = especialidad;
      if (buscar && buscar !== '') {
        where[Op.or] = [
  ...
  crear: async (req, res) => {
    try {
      const { id_especialidad, nombre, apellidos, email, telefono, estado } = req.body;
      const medico = await Medico.create({ id_especialidad, nombre, apellidos, email, telefono, estado: estado || 'Activo' });
  ...
  actualizar: async (req, res) => {
    try {
      ...
      if (!medico) return res.status(404).json({ error: 'Médico no encontrado' });
      const { id_especialidad, nombre, apellidos, email, telefono, estado } = req.body;
      await medico.update({ id_especialidad, nombre, apellidos, email, telefono, estado });
      res.json(medico);
    } catch (error) {
      console.error(error);
